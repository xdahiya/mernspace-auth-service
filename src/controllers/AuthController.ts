...
import { MfaService, UserServicePrimsa, TokenServicePrisma } from "../services";
...
export class AuthController {
    ...
    private userService: UserServicePrimsa;
    ...
    async nonce(req: AuthRequest, res: Response, next: NextFunction) {
        try {
    ...
    async verifySignature(req: Request, res: Response, next: NextFunction) {
        try {
            try {
                ...
                if (!user) {
                    return createHttpError(400, "user not ezists");
                }
                ...
                return createHttpError(401, "web3 signature not verified");
            }
        } catch (err) {
            next(err);
            return;
    ...
    async register(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const userAgent = req.headers["user-agent"] || CONSTANTS.userAgent;
    ...
    async login(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if (user.enable2FA) {
                const mfaToken = this.tokenService.generateMfaLoginToken(
                    user.id,
                );
    ...
    async refresh(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            ...
            await this.tokenService.setdeleteTimeInRefreshToken(
                Number(req.auth.refreshId),
                userId,
            );
            const userAgent = req.headers["user-agent"] || CONSTANTS.userAgent;
            const oldRefreshToken = await this.tokenService.getRefreshTokenById(
                Number(req.auth.refreshId),
            );
            const newRefreshToken = await this.tokenService.persistRefreshToken(
                user.id,
                userAgent,
                oldRefreshToken?.firstCreatedAt,
            );
            const payload: JwtPayload = {
                sub: req.auth.sub,
                role: req.auth.role,
    ...
    async generateMFASetup(
        ...
        try {
            const user = await this.userService.getByIdWithoutTenant(
                Number(req.auth.sub),
            );
            if (!user) {
                const error = createHttpError(400, "User not authorized");
                next(error);
                return;
            }
    ...
    async verifyMFASetup(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const { code } = req.body;
            const user = await this.userService.getByIdWithoutTenant(
                Number(req.auth.sub),
    ...
    async revokeMFA(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const user = await this.userService.getByIdWithoutTenant(
                Number(req.auth.sub),
            );
            ...
            const { message, userPreferences } =
                await this.mfaService.revokeMFA(user);
            return res.status(200).json({
                message,
                userPreferences,
            });
    ...
    async verifyMFAForLogin(
        ...
        try {
            ...
            const result = await this.tokenService.verifyMfaLoginToken(
                id,
                mfaToken,
            );
            if (!user || !result) {
            ...
            const verified = await this.mfaService.verifyMFAForLogin(
                user,
                code,
            );
            if (!verified) {
    ...
    async userPasswordReset(
        ...
        try {
            ...
            const result = this.tokenService.verifyResetPasswordToken(
                user.id,
                token,
            );
            if (!result) {
                const error = createHttpError(400, "token not verified");
                next(error);
                return;
            }
    ...
    async googleOauth(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const { code } = req.body;
            try {
                const { email, name } = await this.userService.getUserDetails(
                    code as string,
                );
    ...
    async socialAccount(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            res.redirect(
                `${Config.CLIENT_UI_DOMAIN!}/api/auth/socialLogin?accessToken=${accessToken}&refreshToken=${refreshToken}`,
            );
